...
public class UbicacionResource {
     ...
     * @return La Ubicacion encontrada. En caso de no existir el id de la
     * Ubicacion buscada se retorna un 404 con el mensaje.
     */
    @GET
    @Path("{id: \\d+}")
     ...
     *
     * @param ubicacionDTO correponde a la representación java del objeto json
     * enviado en el llamado.
     * @return Devuelve el objeto json de entrada que contiene el id creado por
     * la base de datos y el tipo del objeto java.
     */
    @POST
    public UbicacionDetailDTO createUbicacion(UbicacionDetailDTO ubicacionDTO) throws WebApplicationException {
        UbicacionEntity ubicacion = ubicacionDTO.toEntity();
        UbicacionEntity entity = ubicacionLogic.createUbicacion(ubicacion);
     ...
     * @param ubicacionDTO corresponde a al objeto con los cambios que se van a
     * realizar.
     * @return La Ubicacion actualizada. En caso de no existir el id de la
     * Ubicacion a actualizar se retorna un 404 con el mensaje.
     */
    @PUT
    @Path("{id: \\d+}")
    public UbicacionDTO updateUbicacion(@PathParam("id") Long id, UbicacionDTO ubicacionDTO) throws WebApplicationException {
        ubicacionDTO.setId(id);
        UbicacionEntity entity = ubicacionLogic.getUbicacion(id);
        if (entity == null) {
            throw new WebApplicationException("El recurso /ubicaciones/" + id + " no existe.", 404);
        }
        return new UbicacionDetailDTO(ubicacionLogic.updateUbicacion(ubicacionDTO.toEntity()));
    }

    /**
     * DELETE http://localhost:8080/viajesp-web/api/ubicaciones/1
     *
     * @param id corresponde a la Ubicacion a borrar. En caso de no existir el
     * id de la Ubicacion a actualizar se retorna un 404 con el mensaje.
     *
     */
    @DELETE
    @Path("{id: \\d+}")
